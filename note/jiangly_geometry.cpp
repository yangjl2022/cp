...
// from https://ac.nowcoder.com/acm/contest/view-submission?submissionId=63904904
template<class T>
std::vector<Point<T>> convexHull(std::vector<Point<T>> ps) {
    ...
    for (int i = 0; i < n; ++ i) {
        while (h.size() > 1 && cross(h.back() - h.end()[-2], ps[i] - h.back()) >= 0) {
            h.pop_back();
        }
        h.push_back(ps[i]);
    }
    int ver = h.size();
    for (int i = n - 2; i >= 0; -- i) {
        while (h.size() > ver && cross(h.back() - h.end()[-2], ps[i] - h.back()) >= 0) {
            h.pop_back();
        }
        h.push_back(ps[i]);
    }
...
template<class T>
T area(std::vector<Point<T>> ps) {
    T ret = 0;
    int n = ps.size();
    if (n > 1) {
        for (int i = 0; i < n; ++ i) {
            ret += cross(ps[i], ps[(i + 1) % n]);
        }
    }
    return ret / 2;
}
...
template<class T>
Point<T> proj(Line<T> l, Point<T> p) {
    auto vct = l.a - l.b;
    return l.a + vct * (dot(vct, p - l.a) / square(vct));
}
 
template<class T>
Point<T> reflect(Line<T> l, Point<T> p) {
    return proj(l, p) * 2 - p;
}
