...
struct Convex : Polygon<T> {
    ...
    // O(logn) 过点 o 向凸包做两条切线，返回值切点下标 in [0,n-1]
    // 需要保证 o 在凸包外面
...
int main() {
    ...
    cin >> ps;
    Convex<ll> cv(vector<Point<ll>>(ps.begin(), ps.begin() + k));

    const auto& as = cv.p;
    const int sz = cv.n;
    vector<ll> prefCross(as.size());
    for(int i = 0; i + 1 < as.size(); ++i) {
        prefCross[i + 1] = prefCross[i] + as[i].cross(as[i + 1]);
    }

    ll ans = prefCross[sz];
    for(int i = k; i < n; ++i) {
        const Point<ll> o = ps[i];
        if(cv.contain(o)) {
            continue;
        }

        auto [j, k] = cv.tangentByPoint(o);
        if(j > k) {
            k += sz;
        }
        ll tmp = prefCross[sz] - (prefCross[k] - prefCross[j])
            + as[j].cross(o) + o.cross(as[k]);
        ans = max(ans, tmp);
    }
    cout << ans / 2;
    if(ans % 2 == 0) {
...
/*
https://ac.nowcoder.com/acm/contest/28770/C
凸包进阶

题意：给一个凸包，和若干个其他点，判断加入哪个点能让结果的凸包面积最大

- 分别判断每个点加入凸包后面积变化
- 前缀和预处理cross，二分找极点，即可快速计算面积
