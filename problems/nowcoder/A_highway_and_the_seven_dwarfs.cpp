...
int main() {
    ...
    while(cin >> s >> t) {
        ...
        int i = 0, j = 1;
        if(cv.n > 2) {
            auto res = cv.tangentByLine(t - s);
            i = res[0];
            j = res[1];
        }

        if(l.toLeft(cv.p[i]) != l.toLeft(cv.p[j])) {
            cout << "BAD" << endl;
        } else {
            cout << "GOOD" << endl;
        }
...
/*
https://ac.nowcoder.com/acm/contest/28770/B
凸包进阶

题意：q次判断，每次给一条直线，问平面上的点是否分布在直线两边都有

等价于点形成的凸包是否与直线相交

通过二分找到直线的平行线与凸包相切切点，判断切点是否在直线两边即可
O(nlogn + qlogn)
