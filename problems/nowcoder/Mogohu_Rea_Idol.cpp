...
int main() {
    ...
    cin >> ps;
    Convex<ll> ve(ps, true);

    for(int i = 0; i < 2; ++i) {
        cin >> n;
        ps.resize(n);
        cin >> ps;
        ve = MinkowskiSum<ll>(ve, Convex<ll>(ps, true));
    }
...
https://ac.nowcoder.com/acm/contest/28770/K
闵可夫斯基和

题意：给三个凸包，再多次询问点 p，能否在三凸包分别选一个点，使得三角形质心在 p 点

\exist a \in A, b \in B, c \in C, a+b+c = 3p
<=>
3p \in A+B+C

牛客数据不知为什么过不了
https://www.nowcoder.com/discuss/654641244257144832?sourceSSR=users
*/